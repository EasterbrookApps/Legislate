// app.js — integrates debug.js (entry wiring logs), mobile-stable repaint, dual-mode editors
(function () {
  ...
  async function bootstrap() {
    try {
      ...
      engine.bus.on('CARD_DRAWN', async ({ deck, card }) => {
        ...
        try { await modal.open({ title, body: typeof body === 'string' ? body : String(body) }); } catch {}
      });
      ...
      if (rollBtn) {
        ...
        rollBtn.addEventListener('click', async () => {
          ...
          await (modal?.open?.({ title: 'Dice roll', body: `You rolled a ${r}.` }) || Promise.resolve());
      ...
      if (restartBtn) {
        ...
        restartBtn.addEventListener('click', async () => {
          ...
          await (modal?.open?.({ title: 'Play again?', body }) || Promise.resolve());
  ...
  // Early boot: bring up debug panel and traps even before engine init
