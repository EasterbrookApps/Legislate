...
// Scrutiny clock options
const rangesKey = 'si-excludedRanges';
const clockKey = 'si-clockStyle';
function getRanges(){ try { return JSON.parse(localStorage.getItem(rangesKey)) || []; } catch { return []; } }
function saveRanges(r){ localStorage.setItem(rangesKey, JSON.stringify(r)); }
function clockStyle(){ return localStorage.getItem(clockKey) || 'calendar40'; }

function fortyDayDeadline(laid) {
  if (!laid) return null;
  const style = clockStyle();
  if (style === 'calendar40'){
    const d = new Date(laid); d.setDate(d.getDate() + 40); return d;
  }
  // erskine40: count 40 days excluding configured ranges
  let count = 0; let d = new Date(laid);
  const ranges = getRanges().map(r=>({from:new Date(r.from), to:new Date(r.to)}));
  while (count < 40){
    d.setDate(d.getDate()+1);
    const skip = ranges.some(r => d >= r.from && d <= r.to);
    if (!skip) count++;
  }
  return d;
}
...
// Generic fetch with optional proxy retry
async function fetchWithProxy(url, opts={}){
  try{
    ...
    const res2 = await fetch(proxied, opts);
    if (!res2.ok) throw new Error('Proxy HTTP '+res2.status);
    return res2;
  }
}

// Primary source: Linked Data API papers laid (StatutoryInstrument)
...
async function refresh(){
  ...
  try {
    ...
    console.error(e);
    $('#lastSync').textContent = 'Failed to update: ' + e.message + '. If this is a CORS error, set a proxy in Reports → Proxy.';
...
function badge(i){
  ...
  if (i.state==='approved') parts.push(`<span class="badge">Approved</span>`);
  else if (i.state==='withdrawn') parts.push(`<span class="badge">Withdrawn</span>`);
  else if (i.state==='overdue') parts.push(`<span class="badge">40+ days</span>`);
  else if (i.deadline){
    const left = daysBetween(new Date(), i.deadline);
    parts.push(`<span class="badge">${left<=7? '≤7d' : left+'d'}</span>`);
  }
  if (i.jcsi) parts.push(`<span class="badge">JCSI</span>`);
  if (i.slsc) parts.push(`<span class="badge">SLSC</span>`);
  return parts.join(' ');
}
function timeline(i){
  const events = [];
...
// Notifications (due soon) – optional, off by default unless permission already granted
...
// Add the clock settings UI
(function addClockUI(){
  const settingsTab = document.querySelector('#settings .cards');
  const card = document.createElement('article'); card.className='card';
  card.innerHTML = `<h3>Scrutiny clock</h3>
    <p class="muted">Default is a simple 40 calendar days from the laid date. To mimic Erskine May rules, add exclusion ranges (dissolution, prorogation, or joint adjournments &gt; 4 days).</p>
    <div class="row">
      <label class="inline">Clock style
        <select id="clockStyle">
          <option value="calendar40">40 calendar days</option>
          <option value="erskine40">40 days (exclude ranges)</option>
        </select>
      </label>
      <button id="addRange">Add exclusion range</button>
    </div>
    <div id="rangesBox" class="chips"></div>
  `;
  settingsTab.appendChild(card);
  const sty = document.getElementById('clockStyle'); sty.value = clockStyle();
  sty.addEventListener('change', ()=> localStorage.setItem('si-clockStyle', sty.value));
  ...
  function renderRanges(){
    ...
    getRanges().forEach((r,idx)=>{
      ...
      b.addEventListener('click', ()=>{ const cur=getRanges(); cur.splice(idx,1); saveRanges(cur); renderRanges(); });
      box.appendChild(b);
    });
  }
  renderRanges();
  document.getElementById('addRange').addEventListener('click', ()=>{
    const from = prompt('Exclude from (YYYY-MM-DD)'); if (!from) return;
    const to = prompt('Exclude to (YYYY-MM-DD)'); if (!to) return;
    const cur = getRanges(); cur.push({from, to}); saveRanges(cur); renderRanges();
  });
})();
