...
async function loadDecks(){
  ...
  for(const id of ids){
    try{ const res=await fetch('data/cards/'+id+'.json?c='+Date.now()); const arr=await res.json(); Cards.decks[id]={draw:shuffle(arr.slice()),discard:[]}; }
    catch(e){ Cards.decks[id]={draw:[],discard:[]}; }
  }
}
function drawFrom(deckId){
  const d=Cards.decks[deckId]; if(!d) return null;
  if(d.draw.length===0){ d.draw=shuffle(d.discard.slice()); d.discard=[]; }
  const card=d.draw.shift()||null; if(card) d.discard.push(card); return card;
}
function showCard(deckId, card){
  ...
  $('#card-header').textContent=deckId.charAt(0).toUpperCase()+deckId.slice(1);
