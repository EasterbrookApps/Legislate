// Hardened app.js: robust BASE, error logging, safer init
(function(){
  const path = location.pathname;
  const m = path.match(/^(.*\/apps\/si-tracker-v2\.1)/);
  const BASE = m ? m[1] : path.replace(/\/index\.html$/, '').replace(/\/$/, '');
  window.__SI_BASE__ = BASE;
  ...
  function fmtDate(d){ if(!d) return '—'; const o = new Date(d); return isNaN(o)? '—' : o.toLocaleDateString(undefined, {year:'numeric', month:'short', day:'numeric'}); }
  ...
  async function loadData(){
    try{
      ...
      const evs = b.ok ? await b.json() : [];
      if(!Array.isArray(arr)) { console.error('instruments.json is not an array', arr); }
      window.state.data.instruments = Array.isArray(arr) ? arr : [];
      window.state.data.events = Array.isArray(evs) ? evs : [];
      ...
      window.state.data = { instruments: [], events: [] };
    }
  }
  ...
  function renderLaid(){
    const listAll = window.state.data.instruments;
    const list = applyFilters(listAll);
    elView.innerHTML = `<div class="searchbar">
       <input id="q" placeholder="Search titles…" aria-label="Search SIs">
       <button id="exportCsv">Export CSV</button>
      </div>
      <div class="grid">${list.map(card).join('')}</div>
      <div class="footer">Items: ${list.length} (raw: ${listAll.length})</div>`;
    const q = document.getElementById('q');
    q && q.addEventListener('input', ev=>{
      const term = ev.target.value.toLowerCase();
      const filtered = applyFilters(listAll).filter(si=> (si.title||'').toLowerCase().includes(term));
      const grid = document.querySelector('.grid');
      if(grid) grid.innerHTML = filtered.map(card).join('');
      const f = document.querySelector('.footer');
      if(f) f.textContent = `Items: ${filtered.length} (raw: ${listAll.length})`;
    });
    const btn = document.getElementById('exportCsv');
    btn && btn.addEventListener('click', ()=> exportCsv(applyFilters(listAll)));
  }
  ...
  function hookFilters(){
    const cur = document.getElementById('flt-current');
    const com = document.getElementById('flt-commons');
    const clr = document.getElementById('clearFilters');
    if(cur) cur.onchange = (e)=> { window.state.filters.current = e.target.checked; renderLaid(); };
    if(com) com.onchange = (e)=> { window.state.filters.commons = e.target.checked; renderLaid(); };
    if(clr) clr.onclick = ()=> {
      window.state.filters = { current:false, commons:false, status:new Set(), procedure:new Set(), dept:new Set() };
      if(cur) cur.checked = false; if(com) com.checked = false;
  ...
  function initActions(){
    ...
    const d = document.getElementById('densityBtn');
    ...
    if(s) s.onclick = ()=>{ const q = document.getElementById('q'); if(q){ q.focus(); q.select(); } };
  }
  ...
  function render(route){
    ...
    else if(active==='calendar'){ if(window.renderCalendar) window.renderCalendar(); }
